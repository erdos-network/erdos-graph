...
pub async fn scrape_range(
    ...
    println!("Starting XML parsing for years {} to {}...", start_year, end_year);
                ...
                if tag_name == "article" || tag_name == "inproceedings" {
                    ...
                    match parse_dblp_element(&mut reader, tag_name, &dblp_key) {
                        Ok(Some(record)) => {
                            ...
                            if record.year >= start_year as u32 && record.year <= end_year as u32 {
                                ...
                                if records.len() % 1000 == 0 {
                                    println!("Processed {} matching records so far...", records.len());
                                }
                            }
                        }
                        Ok(None) => {
                        ...
                        Err(e) => {
                            ...
                            eprintln!("Warning: Error parsing DBLP element with key '{}': {}", dblp_key, e);
    ...
    println!("DBLP scraping complete. Found {} records in date range {} to {}.", 
             records.len(), start_year, end_year);
    Ok(records)
}
    ...
    loop {
        match reader.read_event_into(&mut buf)? {
            ...
            Event::Eof => {
                ...
                return Err(format!("Unexpected EOF while reading element '{}'", element_name).into());
            }
            _ => {
    ...
    loop {
        match reader.read_event_into(&mut buf)? {
            ...
            Event::Eof => {
                ...
                return Err(format!("Unexpected EOF while skipping element '{}'", element_name).into());
            }
            _ => {

...
/// Orchestrates scraping with optional source-specific modes.
///
/// This is an extended version of `run_scrape` that allows specifying scraping modes
/// for individual sources. This is useful for sources like DBLP that support multiple
/// scraping strategies (e.g., "search" API vs "xml" dumps).
///
/// # Arguments
/// * `start_date` - Start date for scraping
/// * `end_date` - End date for scraping
...
/// `Ok(())` on success, or an error if scraping or ingestion fails
#[coverage(off)]
pub async fn run_scrape_with_modes(
    start_date: DateTime<Utc>,
    end_date: DateTime<Utc>,
    sources: Vec<String>,
    source_modes: HashMap<String, String>,
    datastore: Arc<HelixGraphEngine>,
    config: &Config,
) -> Result<(), Box<dyn std::error::Error>> {
    // Define queue
    ...
    logger::info(&format!(
        "Starting scraping for range {} to {} with modes: {:?}",
        start_date, end_date, source_modes
    ));
    ...
    for src in &sources {
        ...
        let end = end_date;
        let scraper_config = config.scrapers.clone();
        let mode = source_modes.get(src).cloned();
    ...
    // Consumer loop (same as run_scrape)
    let mut ingested_count = 0;
    let mut context = IngestionContext::new(config);

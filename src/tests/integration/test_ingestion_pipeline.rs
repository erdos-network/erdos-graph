...
async fn test_full_pipeline_weekly_mode_dblp() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result =
        orchestrate_scraping_and_ingestion("weekly", sources.clone(), &mut database, &config).await;
...
async fn test_full_mode_from_beginning() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result =
        orchestrate_scraping_and_ingestion("full", sources.clone(), &mut database, &config).await;
...
async fn test_invalid_mode_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result =
        orchestrate_scraping_and_ingestion("invalid_mode", sources.clone(), &mut database, &config)
            .await;
